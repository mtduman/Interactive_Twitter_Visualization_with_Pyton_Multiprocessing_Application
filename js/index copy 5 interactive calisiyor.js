...
//	alert(twData.length)
//	 var wordsX = twData.map(function(d) {
////	      alert("getword"+ d.word+d.size)
//       return {text: d.word, size: d.weight};
//     });

	return words
    .map(function(d) {
//    	alert("inside func " +d.text +d.size)
        return {text: d.word, size: d.weight};
    })
...
function showNewWords(vis, i,words) {
    i = i || 0;

    vis.update(getWords(i ++ % words.length,words))
    setTimeout(function() { showNewWords(vis, i + 1,words)}, 2000)
}
...
//	alert("dur 1")
	...
	showNewWords(myWordCloud, 1 ,words);
